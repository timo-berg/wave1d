...
function plot_series(t, series_data, s, obs_data)
    ...
    for i = 1:nseries
        ...
        title!(p, loc_names[i])
        xlabel!(p, "time [hours]")
        savefig(p, replace("figures/$(loc_names[i])_twin.png", " " => "_"))
...
function plot_state_for_gif(x, s, observed_data)
    # println("plotting a map.")
    #plot all waterlevels and velocities at one time
    xh = 0.001 * s["x_h"]
    p1 = plot(xh, x[1:2:end-1], ylabel="h", ylims=(-3.0, 5.0), legend=false)
    p1 = scatter!(s["xlocs_waterlevel"]./1000, observed_data[1:5])
    xu = 0.001 * s["x_u"]
    p2 = plot(xu, x[2:2:end], ylabel="u", ylims=(-2.0, 3.0), xlabel="x [km]", legend=false)
    p = plot(p1, p2, layout=(2, 1))
...
gif(anim, "figures/animation.gif", fps=20)
