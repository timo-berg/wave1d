...
for i ∈ eachindex(ilocs)
    rmse_sim_ass[i, 1] = rmse(ensemble_mean_5[ilocs[i], :, :], observed_data[i, 2:end])
    rmse_sim_ass[i, 2] = rmse(ensemble_mean_10[ilocs[i], :, :], observed_data[i, 2:end])
    rmse_sim_ass[i, 3] = rmse(ensemble_mean_20[ilocs[i], :, :], observed_data[i, 2:end])
    rmse_sim_ass[i, 4] = rmse(ensemble_mean_50[ilocs[i], :, :], observed_data[i, 2:end])
    rmse_sim_ass[i, 5] = rmse(ensemble_mean_100[ilocs[i], :, :], observed_data[i, 2:end])
end
...
total_plot = plot(plots..., size=(1200,600), dpi=1000)
